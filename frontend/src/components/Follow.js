...
function Follow(props) {
  ...
  return (
    <div className='absolute top-0 left-0 right-0 bottom-0 flex items-center justify-center z-30 backdrop-brightness-50'>
        <div className="bg-white border rounded-xl w-96 h-96 overflow-auto">
            <div className='relative my-2 font-semibold flex justify-center items-center'>
                ...
                <i className="absolute right-2 fas fa-times text-3xl float-right" onClick={()=>hideElem(false)}></i>
            </div>
            <hr />
            {console.log(data)}
            {data && data.map((item)=>{
