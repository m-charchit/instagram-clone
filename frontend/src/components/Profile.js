...
function Profile() {
  ...
  useEffect(() => {
    dispatch(getUser(username)).then((user) => {
      ...
      userPosts
        ? setloading(false)
        : dispatch(fetchUserPosts(user._id, 1)).then(() => {
            setloading(false);
          });
    });
            ...
            <div className="flex">
              ...
              <div className="ml-7 md:ml-24">
                <div className="flex mt-3 md:mt-4  relative flex-col md:flex-row">
                  <div className="flex">
                    <span className="text-3xl font-thin mr-2">
                      {user && user.username}
                    </span>
                    <i
                      className={`fas fa-ellipsis-h text-lg  md:absolute -right-8 mt-1 ${
                        isUser ? "hidden" : ""
                      }`}
                    ></i>
                  </div>
                    ...
                    {isUser ? (
                      ...
                      <>
                        ...
                        {user && user.followingUser ? (
                          ...
                          <button
                            className="bg-blue-600 font-semibold text-white px-5 py-1 rounded "
                ...
                <div className="md:flex mt-7 hidden">
                  <span className="mr-7 flex">
                    <b className="font-semibold mr-1">
                      {userPosts && userPosts.totalDocs}
                    </b>{" "}
                    posts
                  </span>
                  <span
                    className="mr-7 flex cursor-pointer"
          ...
          <div className="border-t md:mt-14">
            ...
            {userPosts && currentUser && (
              <InfiniteScroll
                ...
                dataLength={userPosts.docs.length}
                next={() => {
                  dispatch(
                    fetchUserPosts(currentUser._id, userPosts.nextPage)
                  ).then(() => {
                    setloading(false);
                  });
                  setloading(true);
                }}
                hasMore={userPosts.hasNextPage}
              >
                <div className="flex flex-wrap ">
                  {userPosts.docs.map((post) => {
                    return (
                      <div
                        className="w-1/3 p-px md:px-3 md:pb-6 group cursor-pointer"
                        key={post._id}
                      >
                          ...
                          >
                            ...
                            <div className="w-full h-full absolute  justify-center items-center space-x-7 hidden md:flex">
                              <span className="font-semibold text-white">
                                <i className="fas fa-heart text-xl text-white align-middle"></i>{" "}
                                {post.like}
                              </span>
                              <span className="font-semibold text-white">
                                <i className="fas fa-comment text-xl text-white align-middle"></i>{" "}
                                {post.comments}
                              </span>
                            </div>
                          </div>
